...
This fast version avoids the slow CSV -> transposed memmap -> CSV workflow.
It can also avoid parsing huge environmental CSVs by reading the matching
environmental GeoTIFF rows directly.

It does NOT rerun RF prediction. It first tries to read an existing prediction
GeoTIFF from g*_results/pca_multilevel1_tif. If not available, it falls back to
the existing prediction CSV from g*_results/pca_multilevel1 and uses a temporary
raster-oriented memmap.
...
# Faster path: if pca_multilevel1_tif/prediction*.tif exists, read it directly
# and skip the expensive prediction.csv -> temporary memmap step.
USE_PREDICTION_TIF_IF_AVAILABLE = True
PREDICTION_TIF_DIRNAME = "pca_multilevel1_tif"

# Faster path: read predictor values from environmental GeoTIFF rows instead of
# parsing large environmental CSV rows. Use this only when the TIF layers are
# on the same scale as the global CSV layers used for prediction.
...
CONFIGS = {
    ...
    "g1_abundance": {
        ...
        "output_subdir": "g1_results",
        "tag": "_abundance",
        "features": ["ref_1", "ndvimean", "ECE", "Elevation"],
...
def prediction_tif_path(cfg):
    name = os.path.splitext(cfg["prediction_csv"])[0] + ".tif"
    return os.path.join(
        RESULT_ROOT,
        cfg["output_subdir"],
        PREDICTION_TIF_DIRNAME,
        name,
    )
...
def open_prediction_source(cfg, xsize, ysize, output_dir, model_name):
    pred_tif = prediction_tif_path(cfg)
    if USE_PREDICTION_TIF_IF_AVAILABLE and os.path.exists(pred_tif):
        ds = open_ds(pred_tif)
        if ds.RasterXSize != xsize or ds.RasterYSize != ysize:
            ds = None
            raise RuntimeError(
                f"{model_name}: prediction TIF shape does not match reference grid: {pred_tif}"
            )
        band = ds.GetRasterBand(1)
        print(f"  using prediction TIF for masked outputs: {pred_tif}", flush=True)
        return {"kind": "tif", "ds": ds, "band": band, "mm": None, "tmp_path": None}

    pred_csv = prediction_csv_path(cfg)
    mm, tmp_path = build_prediction_memmap_from_csv(
        pred_csv=pred_csv,
        xsize=xsize,
        ysize=ysize,
    ...
    return {"kind": "memmap", "ds": None, "band": None, "mm": mm, "tmp_path": tmp_path}


def prediction_row(source, y, xsize):
    if source["kind"] == "tif":
        band = source["band"]
        arr = band.ReadAsArray(0, y, xsize, 1).reshape(-1).astype(np.float32)
    ...
    arr = np.asarray(source["mm"][y, :], dtype=np.float32)
    valid = np.isfinite(arr) & (arr != FLOAT_NODATA)
    return arr, valid


def close_prediction_source(source):
    if source is None:
        return
    source["band"] = None
    source["ds"] = None
    if source.get("mm") is not None:
        del source["mm"]
        source["mm"] = None
    tmp_path = source.get("tmp_path")
...
def process_model(model_name, ref_ds, land_band):
    ...
    tag = cfg["tag"]
    features = cfg["features"]
    output_dir = os.path.join(RESULT_ROOT, cfg["output_subdir"], "ridge_mahalanobis_ood_tif_direct2_fast")
    ...
    try:
        ...
        if USE_FEATURE_TIFS_FOR_MD:
            print("  using feature TIF rows for Mahalanobis input", flush=True)
        else:
            for feature in features:
        ...
        if WRITE_MASKED_PREDICTION_TIFS:
            pred_source = open_prediction_source(cfg, xsize, ysize, output_dir, model_name)
        ...
        for y in range(ysize):
            ...
            pred_valid = None
            if use_prediction_input:
                pred_row, pred_valid = prediction_row(pred_source, y, xsize)
        ...
        close_prediction_source(pred_source)
