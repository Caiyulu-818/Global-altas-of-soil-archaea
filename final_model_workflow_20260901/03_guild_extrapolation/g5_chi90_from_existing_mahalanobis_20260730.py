#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fast G5 chi90 risk ratio and mask from an existing Mahalanobis GeoTIFF.

This script does not refit random forest or read environmental/prediction CSV
files. It assumes the input stores D = sqrt(D^2), calculated for the G5 final
predictors: ref_1, ndvimean, biomass and ph.
"""
...
RESULT_DIR = "/public/home/ylwang/Gisdata/TC_New/lucy/uncertainty/g5_results"
MD_TIF = os.path.join(RESULT_DIR, "ridge_mahalanobis_ood_tif_direct1", "mahalanobis_distance.tif")
...
# G5 has p = 4 final predictors: ref_1, ndvimean, biomass and ph.
...
def main():
    ...
    ratio_dataset.SetMetadataItem(
        "RISK_DEFINITION", "ridge_Mahalanobis_distance / sqrt(qchisq(0.90, df=4))"
    )
    ...
    started = time.time()
    try:
        print("=== G5 chi90 from existing Mahalanobis distance ===", flush=True)
        print("Input:", MD_TIF, flush=True)
        print("Threshold sqrt(qchisq(0.90, df=4)): {:.10f}".format(CHI90_THRESHOLD), flush=True)
        ...
        with open(os.path.join(OUTPUT_DIR, "g5_chi90_threshold_and_summary.csv"), "w", newline="") as handle:
            ...
            writer.writerow({
                ...
                "risk_definition": "ridge_Mahalanobis_distance_over_sqrt_qchisq_0.90_df4",
